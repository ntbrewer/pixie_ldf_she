    ...
    for (pugi::xml_node processor = driver.child("Processor"); processor;
        ...
        if (name == "BeamLogicProcessor") {
            ...
            if (front_back_correlation_time == -1) {
                front_back_correlation_time = 3e-7;
                m.warning("Using default front_back_correlation_time = 3e-7",
                          1);
            }
            double dssd_si_correlation_time = 
               processor.attribute("dssd_si_correlation_time").as_double(-1);
            if (dssd_si_correlation_time == -1) {
                dssd_si_correlation_time = 3e-7;
                m.warning("Using default dssd_si_correlation_time = 3e-7",
                          1);
            }
            double delta_energy = 
	    ...
	    double recoil_energy_cut = 
            ...
            int XToffset1 = 
                processor.attribute("XToffset1").as_int(-1);
	        if (XToffset1 == -1) {
                XToffset1 = 10;
                m.warning("Using default XToffset1 = 10", 1);
            }
            int YToffset1 = 
                processor.attribute("YToffset1").as_int(-1);
	        if (YToffset1 == -1) {
                YToffset1 = 10;
                m.warning("Using default YToffset1 = 10", 1);
            }
            int YToffset2 = 
                processor.attribute("YToffset2").as_int(-1);
	        if (YToffset2 == -1) {
                YToffset2 = 10;
                m.warning("Using default YToffset2 = 10", 1);
            }
            double TempCheck =
                processor.attribute("TempCheck").as_double(0);
            if (TempCheck == 0) {
                TempCheck = 0;
                m.warning("Using default TempCheck = 0", 1);
            }
            stringstream ss;
            ...
            vecProcess.push_back(new
                    Dssd4SHEProcessor(front_back_correlation_time,dssd_si_correlation_time,
                                      delta_energy,
		                		      recoil_energy_cut,
                                      high_energy_cut,
                                      low_energy_cut,
