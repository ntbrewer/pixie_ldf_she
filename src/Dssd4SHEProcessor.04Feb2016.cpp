...
/*    DeclareHistogram2D(DD_CHAIN_NUM_FISSION, SC, S9, 
    ...
    DeclareHistogram2D(DD_TOF_O_EVENT, SC, S9,
    ...
    DeclareHistogram2D(DD_MWPC_ENERGY_O_EVENT, SD, S8,
    ...
    DeclareHistogram2D(DD_SI_RAW_EDSSD+0,SD,SB,"Raw Si vs Cal DSSD 1");
...
bool Dssd4SHEProcessor::Process(RawEvent &event)
{
   ...
   plot(D_MWPC_MULTI, mwpc);
	...
	if (hasEscape && event.get_type() == alpha) {
           plot(DD_SI_RAW_EDSSD+escapePos,  event.get_energy()-escapeRaw, escapeRaw);
	}
